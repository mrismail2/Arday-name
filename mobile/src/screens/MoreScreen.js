...
const ICON_TONE = {
  ...
  reports: 'blue', permissions: 'navy', messages: 'blue', settings: 'navy',
};
const toneOf = (c, icon) => {
  const map = {
...
export default function MoreScreen({ navigation }) {
  ...
  const items = profile.nav.filter((k) => k !== 'dashboard' && NAV_META[k]);
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: c.bg }]} edges={['top']}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          {items.map((k, i) => {
            ...
            return (
              <TouchableOpacity
                ...
                activeOpacity={0.6}
                onPress={() => navigation.navigate(STACK_ALIAS[route] || route)}
              >
                <View style={[styles.linkChip, { backgroundColor: t.bg }]}>
                  <Icon name={icon} size={19} color={t.fg} />
                </View>
                <Text style={[styles.linkTxt, { color: c.ink }]}>{label}</Text>
        ...
        <Text style={[styles.version, { color: c.muted2 }]}>{APP.name} · v{APP.version}</Text>
      </ScrollView>
    </SafeAreaView>
  );
}
