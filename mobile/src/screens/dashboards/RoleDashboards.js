...
/* ====================== SUPER ADMIN ====================== */
export function SuperAdminDash() {
  return (
    <>
      {grid([
...
/* ====================== SCHOOL ADMIN ====================== */
export function SchoolAdminDash() {
  const { profile } = useRole();
  const { data: appData } = useAppData();
  // school-scoped: students + recent incidents from the central store
  ...
  return (
    <>
      {grid([
        ...
        { label: 'Firfircoon', value: String(active), icon: 'students', tone: 'green' },
